...
def move(sender, app_data, user_data):
    ...
    if result == 0:
        ...
        dpg.set_value(user_data, 'No files')
    elif result == 2:
        dpg.set_value(user_data, 'Something wrong')

# メインウィンドウ設定
with dpg.window(label='main_window', tag='main_window'):
    dpg.add_button(label='Move osu file' ,callback=move, user_data='result_text')
    dpg.add_separator()
    dpg.add_text(tag='result_text')

# 後処理
dpg.create_viewport(title="osu file move", width=250, height=100)
dpg.setup_dearpygui()
dpg.show_viewport()
